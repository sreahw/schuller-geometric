...
\bd
A \emph{Dynkin diagram} associated to a Cartan matrix is constructed as follows.
\ben
\item Draw a circle for every fundamental root in $\Pi$;
\begin{center}
\begin{tikzpicture}
\draw[fill=white] (2,0) circle[radius=0.15];
\end{tikzpicture}
\end{center}
...
\end{tikzpicture}
\end{center}
\item If $n_{ij}=2,3$, draw an arrow on the lines from the longer root to the shorter root.
\begin{center}
\begin{tikzpicture}
\foreach \x in {0,1} {
\draw (2*3.2*\x+0.62,0.65) node {$|\pi_i|>|\pi_j|$};
\draw (2*3.2*\x+0.75-0.15,0.2) -- (2*3.2*\x+0.75+0.15,0) -- (2*3.2*\x+0.75-0.15,-0.2);
\draw (2*3.2*\x+3.2+0.62,0.65) node {$|\pi_i|<|\pi_j|$};
