...
\bd
Let $(V,\oplus,\odot)$, $(W,\boxplus,\boxdot)$ be vector spaces over the same field $K$ and let $f\cl V\to W$ be a map. We say that $f$ is a \emph{linear map}\index{linear map}\index{map!linear} if for all $u_1,u_2\in V$ and all $\lambda \in K$
\bse
...
\bd
Let $V$ be a vector space over $K$. The \emph{dual}\index{vector space!dual}\index{dual space} vector space to $V$ is $V^*:=\mathrm{Hom}(V,K)$, where $K$ is considered as a vector space over itself.
\ed
...
\bd
Let $T\in T^p_q V$ and $S\in T^r_s V$. We define the \emph{tensor product}\index{tensor!product} of $T$ and $S$ to be $T\otimes S\in T^{p+r}_{q+s}V$ where:
\bi{rl}
...
\bse
\dim \Lambda^nV = \begin{cases} \binom{d}{n} \qquad  &\text{if }\ 1\leq n \leq d\\ 0 &\text{if }\ n > d,\end{cases}
\ese
