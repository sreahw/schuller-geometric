...
\bd
A $C^k$\emph{-manifold}\index{manifold} is a triple $(M,\cO,\mathscr{A})$, where $(M,\cO)$ is a topological manifold and $\mathscr{A}$ is a maximal $C^k$-atlas.
\ed
...
\bse
\begin{tikzcd}
\R^{d} \ar[rrr,"f"] \ar[dd,"\id_{\R^d}"] &&& \R^{d'} \ar[dd,"\id_{\R^{d'}}"]\\
&&\\
\R^{d} \ar[rrr,"\id_{\R^{d'}}\circ f\circ (\id_{\R^d})^{-1}"]&&& \R^{d'}
\end{tikzcd}
\ese
...
\bse
\begin{tikzcd}
U \ar[rr,"x"] \ar[dd,"x"] && x(U) \ar[dd,"\id_{x(U)}"]\\
&&\\
x(U)\se\R^{d} \ar[rr,"\id_{x(U)}\circ x\circ x^{-1}"]&& x(U)\se\R^{d} 
\end{tikzcd}
\ese
...
\bse
\begin{tikzcd}
U \ar[rr,"x^i"] \ar[dd,"x"] && \R \ar[dd,"\id_\R"]\\
&&\\
x(U)\se\R^{d} \ar[rr,"{\id_\R}\circ x^i\circ x^{-1}"]&& \R 
\end{tikzcd}
\ese
